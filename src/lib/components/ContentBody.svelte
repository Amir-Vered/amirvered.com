...
<Bounded tag="article">
    <div class="rounded-2xl border-2 border-stone-800 bg-stone-900 px-4 py-10 md:px-8 md:py-20">
        <Heading tag="h1"> {page.data.title}</Heading>
        <div class="flex gap-4 text-rose-500">
            {#each page.tags as tag}
                <span class="text-xl font-bold">{tag}</span>
            {/each}
        </div>
        <p class="mt-8 border-b border-stone-600 text-xl font-medium text-stone-300">{formatedDate}</p>
