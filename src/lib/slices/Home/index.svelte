<script lang="ts">
	...
	type Props = SliceComponentProps<Content.HomeSlice>;
	...
	<div class="mx-auto w-full max-w-7xl">
		<div class="grid min-h-[65vh] grid-cols-1 items-center md:grid-cols-2">
			...
			<div class="col-start-1 md:row-start-1">
				<h1 class="mb-2 md:mb-2 text-[clamp(3rem,20vmin,13rem)] font-extrabold leading-none text-nowrap" aria-label={slice.primary.first_name + ' ' + slice.primary.last_name}>
					{#if first_name_letters.length && last_name_letters.length}
					<span class="block text-stone-300 tracking-normal">
						{#each first_name_letters as letter} 
							<span class="name-animation inline-block opacity-0">{letter}</span>
						{/each}
					</span>
					<span class="block text-stone-500 -mt-[.2em] tracking-tight">
				...
				<span class = "job-title block bg-linear-to-tr from-rose-800 via-rose-600 to-rose-800 bg-clip-text text-transparent text-2xl font-extrabold uppercase tracking-[.2em] md:text-4xl opacity-0">
