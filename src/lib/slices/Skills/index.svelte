...
<section data-slice-type={slice.slice_type} data-slice-variation={slice.variation} class="overflow-hidden" bind:this={component}>
	<Bounded animated={false} as="div">
		<Heading size="xl" class="mb-8" tag="h2">
			{slice.primary.heading}
		</Heading>
	</Bounded>

	{#each slice.primary.item as {color, name}}
		<div class="tech-row mb-8 flex items-center justify-center gap-4 text-stone-700" aria-label={name || undefined}>
			{#each Array(15) as _, index}
				<span class="item text-8xl font-extrabold uppercase tracking-tighter" 
